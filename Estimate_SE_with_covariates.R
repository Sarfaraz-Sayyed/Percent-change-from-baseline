...
delta_se_mi <- function() {
  sqrt(
    delta_var(type = 1, base=delta_estimatet_m$base_t, 
              post=delta_estimatet_m$post_t, basev=delta_estimatet_m$var_base_t,
              postv=delta_estimatet_m$var_post_t, covar=delta_estimatet_m$cov_t
    ) +
      delta_var(type=1, base=delta_estimatet_f$base_t, 
                post=delta_estimatet_f$post_t, basev=delta_estimatet_f$var_base_t,
                postv=delta_estimatet_f$var_post_t, covar=delta_estimatet_f$cov_t
      ) +
      delta_var(type=1, base=delta_estimatec_m$base_c,
                post=delta_estimatec_m$post_c, basev=delta_estimatec_m$var_base_c,
                postv=delta_estimatec_m$var_post_c, covar=delta_estimatec_m$cov_c
      ) +
      delta_var(type=1, base=delta_estimatec_f$base_c,
                post=delta_estimatec_f$post_c, basev=delta_estimatec_f$var_base_c,
                postv=delta_estimatec_f$var_post_c, covar=delta_estimatec_f$cov_c
      )
  )
}
...
simulation_anal <- function(sim, rho1 = 0.8) {
  for (i in 1:sim) {
    set.seed(123789 + i)
    ...
    colnames(predata_orig) <- c("baseline", "postbase", "sex", "response", "Trt"
    )
...
# Overall sample of 800 patients, 400 in each arm
ss <- 200
out_matrix <- data.frame(simulation_anal(sim = 1000))
...
output4 <- cbind(output3, value = output2$value)
...
cast_data$lbias <- abs(cast_data$lsmean - 0.72)
cast_data$dbias <- abs(cast_data$dmean - 0.72)
cast_data

# Overall sample of 1600 patients, 800 in each arm
ss <- 400
out_matrix1 <- data.frame(simulation_anal(sim = 1000))
output1 <- data.frame(t(out_matrix1))

output2_1 <- data.frame(cbind(output1, rownames(output1)))
colnames(output2_1) <- c("value", "stat")
row.names(output2_1) <- NULL

output3_1 <- data.frame(do.call("rbind", strsplit(as.character(output2_1$stat), "_",
                                                  fixed = TRUE
)))
output4_1 <- cbind(output3_1, value = output2_1$value)
cast_data1 <- cast(output4_1, X1 ~ X2)
cast_data1$lsmean <- -1 * cast_data1$lsmean
cast_data1$lci <- cast_data1$lsmean - 1.96*cast_data1$stderr
cast_data1$uci <- cast_data1$lsmean + 1.96*cast_data1$stderr
cast_data1$dlci <- cast_data1$dmean - 1.96*cast_data1$dstderr
cast_data1$duci <- cast_data1$dmean + 1.96*cast_data1$dstderr
cast_data1$lsmse <- paste0(round(cast_data1$lsmean,3), " (", round(cast_data1$stderr,3),")")
cast_data1$ci <- paste0("(", round(cast_data1$lci,3), ",",round(cast_data1$uci,3),")")
cast_data1$dlsmse <- paste0(round(cast_data1$dmean,3), " (", round(cast_data1$dstderr,3),")")
cast_data1$dci <- paste0("(", round(cast_data1$dlci,3), ",",round(cast_data1$duci,3),")")
cast_data1$lbias <- abs(cast_data1$lsmean - expected1)
