...
delta_var <-
  function(baseline,
           ...
           covar) {
    if (type == 0) {
      dvar <- (((var(postbaseline) / (
        mean(baseline) ** 2
      )) +
        ((
          var(baseline) * (mean(postbaseline) ** 2)
        ) / (
          mean(baseline) ** 4
        )) -
        (
      ...
      dvar <- (((mean(postv) / (mean(
        base
      ) ** 2)) +
        ((
          mean(basev) * (mean(post) ** 2)
        ) / (mean(
          base
        ) ** 4)) -
        (
...
simulation_analysis <- function(ss, sim, rho1 = 0.8) {
  for (i in 1:sim) {
    ...
    for (j in c(20, 30, 40, 50)) {
      ...
      if (i < 2) {
        if (j == 20) {
          flagdat1_20 <- cbind(
            summary(pool(fitimp))[2, 2],
            summary(pool(fitimp))[2, 3],
            ((
            ...
            sqrt(
              delta_var(
                ...
                ,
                1,
                delta_estimatet$base_t,
                delta_estimatet$post_t,
                delta_estimatet$var_base_t,
                delta_estimatet$var_post_t,
                delta_estimatet$cov_t
              ) +
                delta_var(
                  ,
                  ,
                  1,
                  delta_estimatec$base_c,
                  delta_estimatec$post_c,
                  delta_estimatec$var_base_c,
                  delta_estimatec$var_post_c,
                  delta_estimatec$cov_c
                )
            )
          )
        ...
        if (j == 20) {
          flagdat1_20 <- rbind(flagdat1_20,
                               cbind(
                                 summary(pool(fitimp))[2, 2],
                                 summary(pool(fitimp))[2, 3],
                                 ((
                                   mean(delta_estimatet$post_t) /
                                     mean(delta_estimatet$base_t)
                                 ) -
                                   (
                                     mean(delta_estimatec$post_c) /
                                       mean(delta_estimatec$base_c)
                                   )) * 100,
                                 sqrt(
                                   delta_var(
                                     ,
      ...
      if (i < 2) {
        if (j == 20) {
          flagdat2_20 <- cbind(
            coef(summary.lm(fit2))[2, 1],
            coef(summary.lm(fit2))[2, 2],
            ((
              mean(subset_trt(predata1, 1)$postbase) /
                mean(subset_trt(predata1, 1)$baseline)
            ) -
              (
                mean(subset_trt(predata1, 0)$postbase) /
                  mean(subset_trt(predata1, 0)$baseline)
              )) * 100,
            sqrt(
              delta_var(
                subset_trt(predata1, 1)$baseline,
                subset_trt(predata1, 1)$postbase
              ) +
                delta_var(
                  subset_trt(predata1, 0)$baseline,
                  subset_trt(predata1, 0)$postbase
                )
            )
          )
      ...
      if (i < 2) {
        if (j == 20) {
          flagdat3_20 <- cbind(
            coef(summary.lm(fit3))[2, 1],
            coef(summary.lm(fit3))[2, 2],
            ((
              mean(subset_trt(predata2, 1)$postbase1) /
                mean(subset_trt(predata2, 1)$baseline)
            ) -
              (
                mean(subset_trt(predata2, 0)$postbase1) /
                  mean(subset_trt(predata2, 0)$baseline)
              )) * 100,
            sqrt(
              delta_var(
                subset_trt(predata2, 1)$baseline,
                subset_trt(predata2, 1)$postbase1
              ) +
                delta_var(
                  subset_trt(predata2, 0)$baseline,
                  subset_trt(predata2, 0)$postbase1
                )
            )
          )
    ...
    if (i < 2) {
      flagdat <- cbind(
        coef(summary.lm(fit1))[2, 1],
        coef(summary.lm(fit1))[2, 2],
        ((
          mean(subset_trt(predata_orig, 1)$postbase) /
            mean(subset_trt(predata_orig, 1)$baseline)
        ) -
          (
            mean(subset_trt(predata_orig, 0)$postbase) /
              mean(subset_trt(predata_orig, 0)$baseline)
          )) * 100,
        sqrt(
          delta_var(
            subset_trt(predata_orig, 1)$baseline,
            subset_trt(predata_orig, 1)$postbase
          ) +
            delta_var(
              subset_trt(predata_orig, 0)$baseline,
              subset_trt(predata_orig, 0)$postbase
            )
        )
      )
      colnames(flagdat) <-
  ...
  return(
    list(
      ...
      COMP10_dstderr = sqrt(mean(
        data.frame(flagdat2_20)$D_Std_Error ** 2
      )),
      ...
      COMP15_dstderr = sqrt(mean(
        data.frame(flagdat2_30)$D_Std_Error ** 2
      )),
      ...
      COMP20_dstderr = sqrt(mean(
        data.frame(flagdat2_40)$D_Std_Error ** 2
      )),
      ...
      COMP25_dstderr = sqrt(mean(
        data.frame(flagdat2_50)$D_Std_Error ** 2
      )),
      ...
      LOCF10_dstderr = sqrt(mean(
        data.frame(flagdat3_20)$D_Std_Error ** 2
      )),
      ...
      LOCF15_dstderr = sqrt(mean(
        data.frame(flagdat3_30)$D_Std_Error ** 2
      )),
      ...
      LOCF20_dstderr = sqrt(mean(
        data.frame(flagdat3_40)$D_Std_Error ** 2
      )),
      ...
      LOCF25_dstderr = sqrt(mean(
        data.frame(flagdat3_50)$D_Std_Error ** 2
      )),
      ...
      MI10_dstderr = sqrt(mean(
        data.frame(flagdat1_20)$D_Std_Error ** 2
      )),
      ...
      MI15_dstderr = sqrt(mean(
        data.frame(flagdat1_30)$D_Std_Error ** 2
      )),
      ...
      MI20_dstderr = sqrt(mean(
        data.frame(flagdat1_40)$D_Std_Error ** 2
      )),
      ...
      MI25_dstderr = sqrt(mean(
        data.frame(flagdat1_50)$D_Std_Error ** 2
      ))
    )
  )
}
out_matrix1 <- data.frame(simulation_analysis(ss = 400, sim = 1000))
...
output22 <-
  data.frame(do.call("rbind", strsplit(as.character(output21$stat), "_",
                                       fixed = TRUE)))
output23 <- cbind(output22, x3 = output21$value)
